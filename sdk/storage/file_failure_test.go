...
func TestFileOperationStoreRejectsInvalidPersistedState(t *testing.T) {
	tests := []struct {
		name   string
		want   string
		mutate func(*sdk.OperationRecord)
	}{
		{
			name: "state",
			want: "invalid state",
			mutate: func(record *sdk.OperationRecord) {
				record.Operation.State = sdk.OperationState("unknown")
			},
		},
		{
			name: "execution lease",
			want: "execution lease is invalid",
			mutate: func(record *sdk.OperationRecord) {
...
func TestFileDeliveryStoreRejectsInvalidPersistedState(t *testing.T) {
	tests := []struct {
		name   string
		want   string
		mutate func(*sdk.Delivery)
	}{
		{
			name: "state",
			want: "invalid state",
			mutate: func(delivery *sdk.Delivery) {
				delivery.State = sdk.DeliveryState("unknown")
			},
		},
		{
			name: "lease",

...
func (execution *promptExecution) callProvider(
	...
	response, callErr := execution.session.complete(
		ctx,
		call.name,
		call.provider,
		call.request,
...
func (execution *promptExecution) executeTools(
	...
	results := make([]sdk.ToolResult, 0, len(calls))
	for _, rawCall := range calls {
		call, result, err := execution.session.executeTool(
			ctx,
			snapshot,
			turn,
			rawCall,
...
func (execution *promptExecution) decide(
	...
	if err := execution.session.appendTrajectory(
		...
		snapshot.generation,
		trajectoryDecisionPayload{Turn: turn, Action: action},
	); err != nil {
		return sdk.Action{}, err
	}

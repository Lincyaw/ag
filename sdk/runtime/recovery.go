...
func (runtime *Runtime) RecoverExecution(
	...
	defer lease.release()
	executionSnapshot := lease.snapshot
	...
	if checkpoint != nil {
		...
		if checkpoint.Action.Kind == sdk.ActionStop {
			...
			snapshotLease, acquireErr := runtime.acquireSnapshot()
			if acquireErr != nil {
	...
	session.messages = cloneMessages(checkpointMessages(baseCheckpoint))
...
func (runtime *Runtime) checkpointAtOrBefore(
	...
	return decodeTrajectoryCheckpoint(trajectoryID, entry)
}
